...
$baseUrl = 'https://thermenservice-vaillant.at';
...
function isAssetUrl(string $url): bool
{
    ...
    if ($host === 'fonts.googleapis.com' && $path === '/css') {
        return true;
    }
...
function assetCategoryForUrl(string $url): ?string
{
    ...
    if ($extension === 'css' || ($host === 'fonts.googleapis.com' && $path === '/css')) {
        return 'css';
    }
...
function rewriteHtml(
    ...
    foreach (collectUrls($html) as $url) {
        ...
        if ($assetPath !== null) {
            ...
            if (str_contains($url, 'googletagmanager.com/gtm.js?id=')) {
                $replacement .= '?id=';
            }
        ...
        if ($rewritePageLinks && isset($pageUrlMap[$normalized])) {
            $replacement = $pageUrlMap[$normalized];
...
$sitemapUrls = array_filter($sitemapMatches[1], static fn (string $url): bool => str_contains($url, 'page-sitemap') || str_contains($url, 'post-sitemap') || str_contains($url, 'category-sitemap') || str_contains($url, 'elementor-hf-sitemap'));
...
foreach ($sitemapUrls as $sitemapUrl) {
    ...
    foreach ($pageMatches[1] as $pageUrl) {
        if (str_starts_with($pageUrl, $baseUrl)) {
